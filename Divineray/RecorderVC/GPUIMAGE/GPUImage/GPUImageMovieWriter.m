...
- (void)_finishRecordingWithCompletionHandler:(void (^)(void))handler {
    ...
    runSynchronouslyOnContextQueue(_movieWriterContext, ^{
        ...
        if( self->assetWriter.status == AVAssetWriterStatusWriting && ! audioEncodingIsFinished )
        {
            self->audioEncodingIsFinished = YES;
            [self->assetWriterAudioInput markAsFinished];
        }
...
{
    if (_skipAudioCheck) {
        [self _finishRecordingWithCompletionHandler:handler];
        return;
    }
    
    runSynchronouslyOnContextQueue(_movieWriterContext, ^{
        self->willFinish = NO;
        if (self->assetWriter.status == AVAssetWriterStatusCompleted || self->assetWriter.status == AVAssetWriterStatusCancelled || assetWriter.status == AVAssetWriterStatusUnknown)
        {
            self->isRecording = NO;
            if (handler)
                runAsynchronouslyOnContextQueue(self->_movieWriterContext, handler);
        ...
        if (CMTimeCompare(self->previousFrameTime, self->previousAudioTime) == -1) {
            // recoreded audio frame is longer than video frame
            self->willFinish = YES;
            self->completeHandler = handler;
            return;
        }
        self->completeHandler = NULL;
        isRecording = NO;
...
- (void)_processAudioBuffer:(CMSampleBufferRef)audioBuffer {
...
{
    if (_skipAudioCheck) {
        [self _processAudioBuffer:audioBuffer];
        return;
    }
    ...
    if (_hasAudioTrack)
    {
        ...
        {
            runSynchronouslyOnContextQueue(_movieWriterContext, ^{
                if ((self->audioInputReadyCallback == NULL) && (assetWriter.status != AVAssetWriterStatusWriting))
                {
                    [self->assetWriter startWriting];
                }
...
- (void)enableSynchronizationCallbacks;
{
    if (videoInputReadyCallback != NULL)
    {
        ...
        [assetWriterVideoInput requestMediaDataWhenReadyOnQueue:videoQueue usingBlock:^{
            ...
            while( self->assetWriterVideoInput.readyForMoreMediaData && ! _paused )
            {
...
- (void)_newFrameReadyAtTime:(CMTime)frameTime atIndex:(NSInteger)textureIndex {
    if (!isRecording || _paused)
    {
    ...
    {
        runSynchronouslyOnContextQueue(_movieWriterContext, ^{
            if ((self->videoInputReadyCallback == NULL) && (assetWriter.status != AVAssetWriterStatusWriting))
            {
                [self->assetWriter startWriting];
            }
    ...
    runAsynchronouslyOnContextQueue(_movieWriterContext, ^{
        ...
        void(^write)(void) = ^() {
            while( ! self->assetWriterVideoInput.readyForMoreMediaData && ! self->_encodingLiveVideo && ! videoEncodingIsFinished ) {
...
- (void)newFrameReadyAtTime:(CMTime)frameTime atIndex:(NSInteger)textureIndex {
    if (_skipAudioCheck) {
       [self _newFrameReadyAtTime:frameTime atIndex:textureIndex];
        return;
    }
