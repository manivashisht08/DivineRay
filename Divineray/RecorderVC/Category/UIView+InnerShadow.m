...
- (UIView *)createShadowViewWithRadius:(CGFloat)radius andColor:(UIColor *)color inDirection:(NLInnerShadowDirection)direction {
    UIView *shadowView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, [UIApplication sharedApplication].keyWindow.frame.size.width, self.bounds.size.height)];
    ...
    if(direction & NLInnerShadowDirectionTop) {
        ...
        shadow.frame = CGRectMake(0, 0, [UIApplication sharedApplication].keyWindow.frame.size.width, radius);
    ...
    if(direction & NLInnerShadowDirectionBottom) {
        ...
        shadow.frame = CGRectMake(0, self.bounds.size.height - radius, [UIApplication sharedApplication].keyWindow.frame.size.width, radius);
        [shadowView.layer insertSublayer:shadow atIndex:0];
    }
    ...
    if(direction & NLInnerShadowDirectionRight) {
        shadow = [CAGradientLayer layer];
        shadow.colors = colorsArray;
        shadow.frame = CGRectMake([UIApplication sharedApplication].keyWindow.frame.size.width- radius, 0, radius, self.bounds.size.height);
