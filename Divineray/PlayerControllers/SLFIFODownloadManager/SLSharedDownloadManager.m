...
#define SharedDownloadDirectory self.cacheFilesDirectory ?: [NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES).lastObject \
stringByAppendingPathComponent:@"SharedDownloadManager"]

//#define SRFileName(URL) [URL lastPathComponent] // use URL's last path component as the file's name

#define SharedFilePath(URL) [SharedDownloadDirectory stringByAppendingPathComponent:SRFileName(URL)]

#define SharedFilesTotalLengthPlistPath [SharedDownloadDirectory stringByAppendingPathComponent:@"SharedFilesTotalLength.plist"]

@interface SLSharedDownloadManager() <NSURLSessionDelegate, NSURLSessionDataDelegate>
...
@implementation SLSharedDownloadManager
...
- (NSMutableDictionary *)filesTotalLengthPlist {
    if (!_filesTotalLengthPlist) {
        _filesTotalLengthPlist = [NSMutableDictionary dictionaryWithContentsOfFile:SharedFilesTotalLengthPlistPath];
        if (!_filesTotalLengthPlist) {
...
+ (instancetype)sharedManager {
    static SLSharedDownloadManager *downloadManager;
...
- (instancetype)initManager {
    if (self = [super init]) {
        ...
        NSString *downloadDirectory = SharedDownloadDirectory;
        BOOL isDirectory = NO;
      ...
      completion:(void (^)(BOOL success, NSString *filePath, NSError *error))completion
{
    ...
    SLFIFODownloadModel *downloadModel = self.downloadModels[_sharedEncodedFileForURL(URL,_userIDString)];
    ...
    dataTask.taskDescription = _sharedEncodedFileForURL(URL,_userIDString);
...
- (void)URLSession:(NSURLSession *)session dataTask:(NSURLSessionDataTask *)dataTask didReceiveResponse:(NSURLResponse *)response completionHandler:(nonnull void (^)(NSURLSessionResponseDisposition))completionHandler {
    ...
    NSInteger totalLength = (NSInteger) (response.expectedContentLength + [self hasDownloadedLength:downloadModel.URL]);
    downloadModel.totalLength = totalLength;
    self.filesTotalLengthPlist[_sharedEncodedFileForURL(downloadModel.URL,_userIDString)] = @(totalLength);
    [self.filesTotalLengthPlist writeToFile:SharedFilesTotalLengthPlistPath atomically:YES];
...
- (NSInteger)totalLength:(NSURL *)URL {
    ...
    if (!self.filesTotalLengthPlist[_sharedEncodedFileForURL(URL,_userIDString)]) {
        return 0;
    }
    return [self.filesTotalLengthPlist[_sharedEncodedFileForURL(URL,_userIDString)] integerValue];
}
...
- (void)cancelDownloadOfURL:(NSURL *)URL resumeNext:(BOOL)flag {
    ...
    [self.downloadModels removeObjectForKey:_sharedEncodedFileForURL(URL,_userIDString)];
...
- (NSString *)fileFullPathOfURL:(NSURL *)URL {
    ...
    if ([URL.scheme containsString:@"http"]) {
        NSString *filePath = [SharedDownloadDirectory stringByAppendingPathComponent:_sharedEncodedFileForURL(URL, _userIDString)];
        return filePath;
    }
    
    NSString *filePath = [SharedDownloadDirectory stringByAppendingPathComponent:URL.lastPathComponent];
    return filePath;
}
...
- (void)deleteFileOfURL:(NSURL *)URL {
    ...
    [self.filesTotalLengthPlist removeObjectForKey:_sharedEncodedFileForURL(URL,_userIDString)];
    [self.filesTotalLengthPlist writeToFile:SharedFilesTotalLengthPlistPath atomically:YES];
...
- (void)deleteFiles {
    ...
    NSFileManager *fileManager = [NSFileManager defaultManager];
    NSArray *fileNames = [fileManager contentsOfDirectoryAtPath:SharedDownloadDirectory error:nil];
    for (NSString *fileName in fileNames) {
        NSString *filePath = [SharedDownloadDirectory stringByAppendingPathComponent:fileName];
        [fileManager removeItemAtPath:filePath error:nil];
    }
}
...
NSString * _sharedEncodedFileForURL(NSURL *sourceURL , NSString *userId) {
    ...
    NSString *md5Filename = [SLSharedDownloadManager md5HashForString:uniqueFileName];
