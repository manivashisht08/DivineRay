...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        self.hashTagColor = [[UIColor alloc] initWithRed:255.0/255.0 green:255.0/255.0 blue:255.0/255.0 alpha:1.0];
        [self setupLabel];
        [self setupTextView];
...
- (id)initWithCoder:(NSCoder *)coder {
    ...
    if (self) {
        self.hashTagColor = [[UIColor alloc] initWithRed:255.0/255.0 green:196.0/255.0 blue:74.0/255.0 alpha:1.0];
        [self setupLabel];
        [self setupTextView];
        [self setupURLRegularExpression];
    }
...
- (void)setupLabel {
    ...
    _selectionColor = [UIColor clearColor];
    self.hashTagColor = [[UIColor alloc] initWithRed:255.0/255.0 green:255.0/255.0 blue:255.0/255.0 alpha:1.0];
         _attributesText = @{NSForegroundColorAttributeName: self.textColor, NSFontAttributeName: [UIFont fontWithName:@"Poppins-Medium" size:15]};
         _attributesHandle = @{NSForegroundColorAttributeName: [[UIColor alloc] initWithRed:255.0/255.0 green:255.0/255.0 blue:255.0/255.0 alpha:1.0], NSFontAttributeName: [UIFont fontWithName:@"Poppins-Medium" size:15]};
         _attributesHashtag = @{NSForegroundColorAttributeName: self.hashTagColor, NSFontAttributeName: [UIFont fontWithName:@"Poppins-Regular" size:16]};
         _attributesLink = @{NSForegroundColorAttributeName: [[UIColor alloc] initWithRed:255.0/255.0 green:255.0/255.0 blue:255.0/255.0 alpha:1.0], NSFontAttributeName: [UIFont fontWithName:@"Poppins-Medium" size:15]};
...
- (void)determineHotWords {
    // Need a text
    if (_cleanText == nil || ![_cleanText isKindOfClass:[NSString class]])
        return;
...
- (void)determineLinks {
    ...
    [self.urlRegex enumerateMatchesInString:tmpText options:0 range:NSMakeRange(0, tmpText.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        ...
        if ([self->_validProtocols containsObject:protocol.lowercaseString]) {
            [self->_rangesOfHotWords addObject:@{ @"hotWord"  : @(STTweetLink),
                                            @"protocol" : protocol,
